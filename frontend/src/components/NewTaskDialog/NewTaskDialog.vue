    ...
    <div class="space-y-4" v-if="newTask">
      ...
      <div class="grid grid-cols-2 gap-2">
        ...
        <Combobox
          ...
          @update:modelValue="newTask.doc.journey_milestone = ''"
        />
        <Dropdown class="w-full" :options="milestoneDropdownOptions">
          <Button class="w-full">
...
let { options: milestoneOptions } = useMilestoneOptions(() => newTask.value?.doc.project)
const selectedMilestone = computed(() => String(newTask.value?.doc.journey_milestone || ''))
...
function changeMilestone(option: string) {
  ...
  newTask.value.doc.journey_milestone = option === '<no_milestone>' ? '' : String(option)
}
