  ...
  <div class="flex h-full flex-1" v-if="task.doc">
    <div class="w-full flex-1">
      <div class="relative p-3 sm:p-6">
        ...
        <div class="mt-8 flex flex-wrap items-center gap-2 sm:hidden">
          ...
          />
          <Dropdown :options="milestoneDropdownOptions">
            <Button>
    ...
    <div class="hidden w-[20rem] shrink-0 border-l sm:block">
      <div class="grid grid-cols-2 items-center gap-y-6 p-6 text-base text-ink-gray-6">
        ...
        <div>Milestone</div>
        <div>
          <Dropdown :options="milestoneDropdownOptions" align="end">
            <Button>
              {{ selectedMilestoneLabel }}
            </Button>
          </Dropdown>
...
import { useMilestoneOptions } from '@/data/milestones'
...
const { options: milestoneOptions } = useMilestoneOptions(() => task.doc?.project)
const selectedMilestone = computed(() => String(task.doc?.journey_milestone || ''))
const selectedMilestoneLabel = computed(() => {
  return milestoneOptions.value.find((option) => option.value === selectedMilestone.value)?.label || 'Milestone'
})
const milestoneDropdownOptions = computed(() =>
  milestoneOptions.value.map((option) => ({
    label: option.label,
    onClick: () => changeMilestone(option.value),
  })),
)
...
function changeSpace(option: string) {
  ...
  task.doc.journey_milestone = ''
  task.setValue.submit({ project: option, journey_milestone: '' }).then(updateRoute)
}

function changeMilestone(option: string) {
  if (!task.doc) return
  task.doc.journey_milestone = option === '<no_milestone>' ? '' : String(option)
  task.setValue.submit({ journey_milestone: task.doc.journey_milestone })
}
